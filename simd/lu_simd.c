...
void daxpy ( double *a, double *b, long n, double alpha )
{
	long i;
	double arr[2];
	__m128d n1, n2, n3, n4, n5;
	n3= _mm_loadu_pd(&alpha);
...
void lu ( long n, long bs )
{
	...
	for ( k=0, K=0; k<n; k+=bs, K++ ) {
		...
		I=K+1;
                J=K+1;
		/* divide row k by diagonal block */
		D = & ( a[k+k*n] );
               #pragma omp parallel shared(D) 
		{
		#pragma omp for schedule(static)  private(il,A) lastprivate(I)  nowait
		for ( i=kl; i<n; i+=bs ) {
			I++;
			il = i + bs;
			if ( il > n ) {
		...
		/* modify column k by diagonal block */

            #pragma omp for schedule(static)  private(jl,A) lastprivate(J) 
		for ( j=kl; j<n; j+=bs ) {
			J++;
			jl = j+bs;
			if ( jl > n ) {
		...
		I=K+1;
           ...
           #pragma omp for schedule(static) private(il,A) 
		...
		for ( i=kl; i<n; i+=bs ) {
			...
			A = & ( a[i+k*n] );
			J=K+1;
                   #pragma omp parallel for schedule(static) private(jl,B,C) 
			for ( j=kl; j<n; j+=bs ) {
				J++;
				jl = j + bs;
				if ( jl > n ) {
